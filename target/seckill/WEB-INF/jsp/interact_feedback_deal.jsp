...
<header class="mui-bar mui-bar-nav">
    ...
    <h1 class="mui-title">投诉(维修)反馈汇总表</h1>
...
<div class="mui-content">
    ...
    <div class="form-group-sm" style="margin-top: 30px;margin-left: 30px;">
        ...
        <label for="startDate" style="float: left;" class=" control-label">巡检人查询</label>
        <div class="col-md-1">
    ...
    <div class="table-responsive" id="table" style="clear: both;margin-top: 30px">
        <table class="table">
            ...
            <c:forEach items="${feedbackList}" var="list">
                <tr <c:if test="${list.isDeal==0}"> style="color: #ff0000" </c:if>>
                    <td>${list.id}</td>
                    <td>${list.userName}</td>
                    <td>${list.title}</td>
                    <td>
                        <c:if test="${list.type==1}"> 报修 </c:if>
                        <c:if test="${list.type==0}"> 投诉 </c:if>
                    </td>
                    <td>${list.date}</td>
                    <td>
                        <c:if test="${list.isDeal==1}"> 完成 </c:if>
                    </td>


                    <td><a style="color: #1d4499" onclick="skipFinanceAdd('${list.id}')">详情</a></td>
...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function () {
        ...
        $("#page-container-static-big").on("pageChanged", function (event, params) {
            ...
            if (query == 'query') {
                ...
                var url = '/seckill/interact_feedback/' + userId + '/' + (params.pageNum - 1) + '/' + userName + '/' + startDate + '/' + endDate;

                $(window).attr('location', url);

            } else {

                var url = '/seckill/interact_feedback/' + userId + '/' + (params.pageNum - 1) + '/deal';
                $(window).attr('location', url);
    ...
    function skipFinanceAdd(id) {
        mui.openWindow({
            url: '/seckill/interact_feedback_deal/' + id + '/feedback/insert'
    ...
    function queryFinance() {
        ...
        var url = '/seckill/interact_feedback/' + userId + '/0/' + userName + '/' + startDate + '/' + endDate;
