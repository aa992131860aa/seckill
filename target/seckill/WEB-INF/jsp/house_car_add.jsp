...
<header class="mui-bar mui-bar-nav">
    ...
    <c:choose>
        <c:when test="${id>-1}">
            <h1 class="mui-title">车位修改</h1>
        </c:when>
        <c:otherwise>
            <h1 class="mui-title">车位新增</h1>
        </c:otherwise>
...
<div class="mui-content">
    ...
    <form id="form" data-toggle="validator" class="form-horizontal" role="form"
        ...
        <div class="form-group">
            ...
            <select id="carType" class="col-sm-6 selectpicker">
                <c:forEach items="${carList}" var="d">
                    <option <c:if test="${d.id==carType}"> selected="selected" </c:if>
                            value="${d.id}">${d.name}</option>
                </c:forEach>
            ...
            <div class="col-xs-offset-4 col-sm-offset-6 col-sm-4">
                ...
                <c:choose>
                    <c:when test="${id>-1}">
                        ...
                        <button type="button" class="mui-btn mui-btn-primary" onclick="modify()"> 修改车位</button>
...
<script type="text/javascript" charset="utf-8">
    ...
    function modify() {
        ...
        var url = "/seckill/house_car/" + houseId + "/" + userId + "/" + goodsNameId + "/" + loctionL + "/" + useArea
            + "/" + num + "/" + startTime + "/" + endTime + "/" + goodsStatusId + "/" + floor + "/" + carType + "/" + chartLoc + "/modify";
        //alert(url)
    ...
    function confirm() {
        ...
        var url = "/seckill/house_car/" + userId + "/" + goodsNameId + "/" + loctionL + "/" + useArea + "/" + num + "/" + startTime
            + "/" + endTime + "/" + goodsStatusId + "/" + floor + "/" + carType + "/" + chartLoc + "/add";
        $.post(url, {}, function (result) {
            //alert(result);
            if (result >= 1) {
                sessionStorage.setItem("need-refresh", true);
