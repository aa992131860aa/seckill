...
<div class="mui-content">
    ...
    <div class="form-group-sm" style="margin-top: 30px;margin-left: 30px;">
        ...
        <label for="number" style="float: left;" class="form-label">车牌号查询</label>
        <div class="col-md-1">
            <input style="display: inline;height: 30px;" type="text" value="${number}" class="form-control input-sm"
                   id="number">
        </div>
        ...
        <table class="table">
            ...
            <c:forEach items="${contractList}" var="list">
                <tr>
                    ...
                    <td>
                        <a
                                onclick="detailContract('${list.uuid}')">详情 | </a>
                        <a style="
                        <c:choose>
                            <c:when test="${is_admin=='1'}"></c:when>
                        <c:otherwise>
                        <c:if
                           ...
                           onclick="updateContract('${list.uuid}')"><c:if test="${list.isOk==-1}">重新审批</c:if><c:if test="${list.isOk==0||list.isOk==1}">续签</c:if> | </a>
                        <a style="
                        <c:choose>
                            <c:when test="${is_admin=='1'}"></c:when>
                        <c:otherwise>
...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function () {
        ...
        $("#page-container-static-big").on("pageChanged", function (event, params) {
            ...
            if (query == 'query') {
                ...
                $(window).attr('location', '/seckill/contract_car/' + userId + '/' + (params.pageNum - 1) + '/' + code + '/' + linkMan + '/' + no + '/' + startDate + '/' + endDate + '/' + number + '/query/noExport');

            } else {

                $(window).attr('location', '/seckill/contract_car/' + userId + '/' + (params.pageNum - 1) + '/noExport');
    ...
    function excelExport() {
        ...
        if (query == 'query') {
            ...
            var url = '/seckill/contract_car/' + userId + '/' + page + '/' + code + '/' + linkMan + '/' + no + '/' + startDate + '/' + endDate + '/' + number + '/query/export';
            ...
            var url = '/seckill/contract_car/' + userId + '/' + page + '/export';
            mui.openWindow({
                url: url
    ...
    function skipContractHouse() {
        mui.openWindow({
            url: '/seckill/contract_car/insert'
        })

    }

    function updateContract(uuid) {
        var url = "/seckill/contract_car/" + uuid + "/update";
        mui.openWindow({
            url: url
    ...
    function detailContract(uuid) {
        var url = "/seckill/contract_car/" + uuid + "/normal/detail";
        mui.openWindow({
            url: url
    ...
    function updateCustom(uuid) {
        ...
        var url = "/seckill/contract_car/" + uuid + "/0/del/"+date+"/0";
        mui.openWindow({
            url: url
    ...
    function queryContract() {
        ...
        var endDate = $("#endDate").val();
        var rs = "fantasy";
        if (code == '' && linkMan == '' && no == '' && number == '' && startDate == '' && endDate == '') {
            mui.toast('请输入查询条件');
        } else {
            if (code == '') {
                code = rs;
            ...
            var url = "/seckill/contract_car/" + userId + '/0/' + code + '/' + linkMan + '/' + no + '/' + startDate + '/' + endDate + '/' + number + "/query/noExport";
            mui.openWindow({
                url: url
