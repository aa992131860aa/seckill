...
<div class="mui-content">
    ...
    <form id="form" data-toggle="validator" class="form-horizontal" role="form"
        ...
        <div class="form-group">
            <label for="name" class="col-sm-2 control-label">内容类别选择</label>

            投诉类
            <input value="0"
            <c:if test="${insert=='insert'}"> checked</c:if>
            <c:if test="${feedback.type==0 && insert=='update'}"> checked </c:if>
            ...
            <c:if test="${feedback.type==1 && insert=='update'}"> checked </c:if>
        ...
        <div class="form-group">
            <label for="name" class="col-sm-2 control-label">内容填写</label>
            <input type="text" style="width: 60%;" value="${feedback.title}" id="title"/>


        </div>
        <div class="form-group">
            <label for="name" style="float: left;width: 80%;margin-bottom: 10px"
                   class="col-sm-2 control-label">详情描述</label>


            <button type="button" style="float: right;"
                    class="mui-btn mui-btn-outlined mui-btn mui-btn-primary"
                    onclick="addContent()"> 新增
            </button>

        </div>
        <div class="form-group">
            <div id="content">
                <c:forEach items="${feedback.contentList}" var="list">
        ...
        <div class="form-group">
            <label for="sale" class="col-sm-2 control-label" style="font-size: 12px">拍照上传(点击图片删除)</label>
        ...
        <div class="form-group">
            <div id="photolist" style="float: left">
                ...
                <c:forEach items="${feedback.urlsList}" var="list">
        ...
        <div class="form-group">
            <div id="dealContent" class="col-sm-offset-2">
                ...
                <c:forEach items="${feedbackDealList}" var="list">
...
<script type="text/javascript" charset="utf-8">
    ...
    function confirm() {
        var flag1 = "@@";
        var isNormal = $("input[name='normal']:checked").val();
        var title = $("#title").val();
        var content = "";
        var srcUrls = "";
        var insert = '${insert}';
        ...
        $(".new_photo").each(function (index) {
            ...
            if (srcurl.indexOf("/download/") != -1) {
                ...
                if (len == 0) {
                    ...
                    var url = "/seckill/interact_feedback/" + id + "/" + title + "/" + isNormal + "/" + content + "/" + srcUrls + "/" + usrName + "/" + insert;
        ...
        if ($(".new_photo").length == 0) {
            ...
            var url = "/seckill/interact_feedback/" + id + "/" + title + "/" + isNormal + "/" + content + "/" + srcUrls + "/" + userName + "/" + insert;
