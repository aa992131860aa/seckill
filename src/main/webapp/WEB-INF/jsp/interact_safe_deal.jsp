    ...
    <div class="table-responsive" id="table" style="clear: both;margin-top: 30px">
        <table class="table">
            ...
            <c:forEach items="${safeList}" var="list">
                <tr <c:if test="${list.isNormal==1}"> style="color: #ff0000" </c:if>>
                    <td>${list.id}</td>
                    <td>${list.dot}</td>
                    <td>
                        <c:if test="${list.isNormal==1}"> 异常 </c:if>
                        <c:if test="${list.isNormal==0}"> 正常 </c:if>
                    </td>
                    <td>
                        <c:if test="${list.isDeal==1}"> 完毕 </c:if>
                    </td>
...
<script type="text/javascript" charset="utf-8">
    ...
    function queryFinance() {
        ...
        var url = '/seckill/interact_safe_deal/' + userId + '/0/safe/safe/' + userName + '/' + startDate + '/' + endDate;
