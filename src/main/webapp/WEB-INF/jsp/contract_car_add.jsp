                            ...
                            <c:forEach items="${contract.tableList1}" var="b">
                                ...
                                <tr>
                                    <td><input
                                            ...
                                            onclick="search(this)" id="${b.table0}" type="text" value="${b.table1}"
                                            style=""
                                            name="loc1"/></td>
                                        ...
                                        <%--name="useArea1"/></td>--%>
                                    <td>
                                        <select style="width: 120px;padding-left: 40px;"
                                                class="form-control "
                                                id="${b.table0}nu" onchange="noChange(this)" type="text"
                                                name="number1">
                                            <c:forEach items="${carRegList}" var="c" varStatus="cc">
                                                <option
                                                        <c:if test="${b.table11 == c.no}"> selected="selected" </c:if>
                                                        value="${cc.index}">${c.no}</option>
                                            </c:forEach>
                                        </select>
                                    ...
                                    <td>
                                        <select style="width: 120px;padding-left: 40px;background-color: white" disabled
                                                class="form-control "
                                                id="${b.table0}pe" type="text"
                                                name="person1">
                                            <c:forEach items="${carRegList}" var="c" varStatus="cc">
                                                <option
                                                        <c:if test="${b.table8 == c.name}"> selected="selected" </c:if>
                                                        value="${cc.index}">${c.name}</option>
                                            </c:forEach>
                                        </select>

                                            <%--<input disabled="disabled"--%>
                                            <%--style="border-bottom: 1px solid #dbdbdb;border-top:0px;border-left:0px;border-right:0px;border-radius:0;text-align:center"--%>
                                            <%--id="uId " type="text" value="${b.table8}"--%>
                                            <%--name="person1"/>--%>
                                    </td>

                                    <td nowrap="nowrap">

                                        <select style="width: 160px;padding-left: 40px;background-color: white" disabled
                                                class="form-control "
                                                id="${b.table0}ph" type="text"
                                                name="phone1">
                                            <c:forEach items="${carRegList}" var="c" varStatus="cc">
                                                <option
                                                        <c:if test="${b.table9 == c.phone}"> selected="selected" </c:if>
                                                        value="${cc.index}">${c.phone}</option>
                                            </c:forEach>
                                        </select>

                                            <%--<input disabled="disabled"--%>
                                            <%--style="border-bottom: 1px solid #dbdbdb;border-top:0px;border-left:0px;border-right:0px;border-radius:0;text-align:center"--%>
                                            <%--id="uId " type="text" value="${b.table9}"--%>
                                            <%--name="phone1"/></td>--%>

                                    <td>
                                        <input disabled="disabled"
                                               style="border-bottom: 1px solid #dbdbdb;border-top:0px;border-left:0px;border-right:0px;border-radius:0;text-align:center"
                                               id="uId " type="text" value="${b.table10}"
                                               name="num1"/></td>
                                    <td>
                                        ...
                                        <table class="table">
                                            ...
                                            <c:forEach items="${contract.tableList11}" var="b">
                                                <tr id="${b.table6}t11">
                                                    ...
                                                    <td style="display: none"><input disabled="disabled"
                                                                                     style="border-bottom: 1px solid #dbdbdb;border-top:0px;border-left:0px;border-right:0px;border-radius:0;text-align:center"
                                                                                     type="text" name="car11"
                                                                                     value="${b.table4}"/></td>
                                                    <td style="display: none"><input disabled="disabled"
                                                                                     style="border-bottom: 1px solid #dbdbdb;border-top:0px;border-left:0px;border-right:0px;border-radius:0;text-align:center"
                                                                                     type="text" name="linkMan11"
                                                                                     value="${b.table5}"/></td>
                                                </tr>
                                            </c:forEach>
                                            </tbody>
...
<script type="text/javascript" charset="utf-8">
    ...
    $(function () {
        ...
        $("#code_auto").autocomplete({
            ...
            select: function (event, ui) { // 选中某项时执行的操作
                ...
                if (ui.item.carRegList.length > 0) {
                    var linkMan11 = '';
                    var car11 = '';
                    for (var i = 0; i < ui.item.carRegList.length; i++) {
                        linkMan11 += ui.item.carRegList[i].name + ',';
                        car11 += ui.item.carRegList[i].no + ',';
                    }
                    $("#linkMan11").val(linkMan11);
                    $("#car11").val(car11);
                } else {
                    $("#linkMan11").val('');
                    $("#car11").val('');
    ...
    function search(obj) {
        ...
        $("#" + obj.id).autocomplete({
            // searchStreedRoad_auto 输入框id
            source: function (request, response) {
                var url = '/seckill/contract_car/' + request.term + '/' + code_auto + '/' + remark + '/' + customId + '/loc';
                console.log(url)
                    ...
                    response(
                        $.map(res, function (item) { // 此处是将返回数据转换为 JSON对象，并给每个下拉项补充对应参数
                            ...
                            return {
                                ...
                                phone: item.phone,
                                number: item.number,
                                carRegLists: item.custom.carRegList
            ...
            select: function (event, ui) { // 选中某项时执行的操作
                ...
                var car11s = $("#car11").val().split(',');
                var linkMan11s = $("#linkMan11").val().split(',');
                console.log('car11:' + car11s.length + ',' + loc11)
                var car11 = '';
                if (car11s.length > loc11) {
                    car11 = car11s[loc11];
                } else {
                    car11 = car11s[0];
                }
                var linkMan11 = '';
                if (linkMan11s.length > loc11) {
                    linkMan11 = linkMan11s[loc11];
                } else {
                    linkMan11 = linkMan11s[0];
                }
                //加载模态框
                addWork11(tid11, ui.item.loc, ui.item.floor, ui.item.name, car11, linkMan11);


                setTotal1()


                $("#" + peid).val(ui.item.person)


                $("#" + phid).val(ui.item.phone)


                //车辆的下拉选择
                for (var i = 0; i < ui.item.carRegLists.length; i++) {
                    $("#" + nuid).append("<option value='" + i + "'>" + ui.item.carRegLists[i].no + "</option>");
                    $("#" + peid).append("<option value='" + i + "'>" + ui.item.carRegLists[i].name + "</option>");
                    $("#" + phid).append("<option value='" + i + "'>" + ui.item.carRegLists[i].phone + "</option>");
    ...
    function noChange(obj) {
        ...
        var peId = obj.id.split('nu')[0] + 'pe';
        var phId = obj.id.split('nu')[0] + 'ph';

        $("#" + peId).val(obj.selectedIndex)
        $("#" + phId).val(obj.selectedIndex)

    }

    function addWork() {
        if ($("#code_auto").val() == '') {
            mui.toast('请填写客户名称 ', {duration: 'short', type: 'div'})
            return;
        }
        if (!dateOk()) {
            return;
        ...
        var line = '<td><input   style="border-bottom: 1px solid #dbdbdb;border-top:0px;border-left:0px;border-right:0px;border-radius:0;text-align:center;" onclick="search(this)" id="' + lId + '" type="text" value=""  style="" name="loc1"  /></td>' +
            ...
            '<td ><select onchange="noChange(this)" style="width: 120px;padding-left: 40px;" class="form-control selectpicker"  id="' + nuId + '" type="text" name="number1"  ></select></td>' +
            select +
            '<td ><select disabled style="width: 120px;padding-left: 40px;background-color: white" class="form-control selectpicker" id="' + peId + '" type="text" name="person1"  /></td>' +
            '<td nowrap="nowrap"><select  disabled  style="width: 160px;padding-left: 40px;background-color: white;" class="form-control selectpicker" id="' + phId + '" type="text" name="phone1"  /></td>' +
            '<td ><input disabled="disabled"  style="border-bottom: 1px solid #dbdbdb;border-top:0px;border-left:0px;border-right:0px;border-radius:0;text-align:center" id="' + nId + '" type="text" name="num1"  /></td>' +
            time +
    ...
    function confirm() {
        ...
        var person1 = '';
        // $("input[name='person1']").each(function (index, item) {
        //     person1 += $(this).val() + flag1;
        // });
        var phone1 = '';
        // $("input[name='phone1']").each(function (index, item) {
        //     phone1 += $(this).val() + flag1;
        // });

        var number1 = '';
        // $("input[name='number1']").each(function (index, item) {
        //     number1 += $(this).val() + flag1;
        // });

        $("input[name='loc1']").each(function (index, item) {
            loc1 += $(this).val() + flag1;
            id1 += item.id + flag1;
            person1 += $("#" + item.id + "pe").find("option:selected").text() + flag1;
            phone1 += $("#" + item.id + "ph").find("option:selected").text() + flag1;
            number1 += $("#" + item.id + "nu").find("option:selected").text() + flag1;
        ...
        var num1 = '';
        $("input[name='num1']").each(function (index, item) {
            num1 += $(this).val() + flag1;
        });


        table1 = id1 + flag2 + loc1 + flag2 + time1 + flag2 + useArea1 + flag2 + cash1 + flag2 + money1 + flag2 + price1 + flag2 + goodsStatus1 + flag2 + person1 + flag2 + phone1 + flag2 + num1 + flag2 + number1;
        ...
        var car11 = '';
        $("input[name='car11']").each(function (index, item) {
            car11 += $(this).val() + flag1;
        });
        var linkMan11 = '';
        $("input[name='linkMan11']").each(function (index, item) {
            linkMan11 += $(this).val() + flag1;
        });


        table11 = loc11 + flag2 + floor11 + flag2 + money11 + flag2 + name11 + flag2 + car11 + flag2 + linkMan11;
        ...
        var url = '';
        if (isOk == -1) {
            var url = "/seckill/contract_car/" + uuid + "/" + main + "/" + table1
                + "/" + table2 + "/" + table2H + "/" + table3 + "/" + table11 + "/" + table33 + "/update";

        } else {
            url = "/seckill/contract_car/" + main + "/" + table1
