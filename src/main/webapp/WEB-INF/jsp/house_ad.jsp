...
<header class="mui-bar mui-bar-nav">
    ...
    <c:if test="${role4=='fantasy'}">
        <h1 class="mui-title">查询广告资源</h1>
    </c:if>
    <c:if test="${role4!='fantasy'}">
        <h1 class="mui-title">广告资源管理</h1>
    </c:if>
                ...
                <c:forEach items="${houseList}" var="list">
                    <tr>
                        ...
                        <td <c:choose> <c:when test="${is_admin=='1'}"></c:when><c:otherwise><c:if
                                test="${!role4.contains('删除')}"> style="display: none" </c:if></c:otherwise></c:choose>>
                            <a
                                    onclick="deleteHouse(${list.id})" style="color: #f00;">删除</a></td>

                        <td <c:choose> <c:when test="${is_admin=='1'}"></c:when><c:otherwise><c:if
                                test="${!role4.contains('编辑')}"> style="display: none" </c:if></c:otherwise></c:choose>>
                            <a onclick="modifyHouse('${list.id}','${list.userId}','${list.goodsNameId}'
                                    ,'${list.location}','${list.useArea}','${list.num}'
                                    ,'${list.startTime}','${list.endTime}','${list.goodsStatusId}','${list.floor}','${list.adId}','${list.adName}','${list.chartLoc}')">详情</a>
                        </td>
                    </tr>
                </c:forEach>
                <tr>
...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function () {
        ...
        $("#page-container-static-big").on("pageChanged", function (event, params) {
            ...
            if (query == 'query') {
                ...
                $(window).attr('location', '/seckill/house/' + userId + '/' + (params.pageNum - 1) + '/house_ad/noExport');
            }
    ...
    function excelExport() {
        ...
        if (query == 'query') {
            ...
            var url = '/seckill/house/' + userId + '/' + page + '/house_ad/export';
            mui.openWindow({
                url: url
    ...
    function skip_house_add() {
        mui.openWindow({
            url: '/seckill/house/' + userId + '/house_ad_add/add',
            id: 'department'
        })

    }

    function modifyHouse(id, userId, goodsNameId, loctionL, useArea, num, startTime, endTime, goodsStatusId, floor, adType,adName,chartLoc) {
     if(chartLoc==''){
         chartLoc = 'fantasy';
     }
        var url = "/seckill/house_ad/" + id + "/" + userId + "/" + goodsNameId + "/" + loctionL + "/" + useArea + "/" + num + "/" + startTime
            + "/" + endTime + "/" + goodsStatusId + "/" + floor + "/" + adType +"/"+adName+"/"+chartLoc+ "/info";
        mui.openWindow({
            url: url,
            id: 'department',
